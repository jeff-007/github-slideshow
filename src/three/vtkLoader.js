...
Three.VTKLoader.prototype = {
  ...
  load: function(url, callback) {
    ...
    request.addEventListener('load', function(event) {
      // 对服务器加载下来的数据进行解析, 解析完成后，发一个load事件，表示数据解析完成
      // 如果设置了回调函数，那么调用回调函数
      const geometry = scope.parse(event.target.responseText);
  ...
  parse: function(data) {
    ...
    // geometry中每一个面的重心，取平面三个点x, y, z轴坐标的平均值
    geometry.computeCentroids();

    // 来计算每一个面归一化后的法向量，法向量垂直于面。计算之后的法向量被存放在了face.normal中。法向量与模型受光情况有关
    geometry.computeFaceNormals();

    // 计算每一个顶点的法向量
    geometry.computeVertexNormals();

    // 计算一个可以包围geometry的一个椭圆，中心点就是geometry的中心，sphere的最大半径是离中心点最远的那一个点
